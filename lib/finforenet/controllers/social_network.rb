      ...
      module InstancesMethods
			...
			private
			  ...
			  def get_default_callback
			    ...
			    single_token_param = "finfore_token=#{current_user.single_access_token}"
			    auth_token_param = "finfore_token=#{params[:finfore_token]||params[:auth_token]}"
			    url_query = current_user ? single_token_param : auth_token_param
			  ...
			  def linkedin_profile(access_token)
				  access_token.get('/v1/people/~:(id,first-name,last-name)').body
				end
				
				def google_profile(access_token)
					access_token.get("http://www.google.com/m8/feeds/contacts/default/full?max-results=1&alt=json").body
				end
				
				def twitter_profile(access_token)
					access_token.get('/1/account/verify_credentials.json').body
				end
		
			  def get_profile_network(access_token,api,stored_data)
			    if api.category == "linkedin"
			      person = Nokogiri::XML::Document.parse(linkedin_profile(access_token)).xpath('person')
			      person_hash = {'username' => person.xpath('id').text, 
				                   'provider' => api.category, 
				                   'name'      => "#{person.xpath('first-name').text} #{person.xpath('last-name').text}"}
			    elsif api.category == "twitter"
			      person = Yajl::Parser.parse(twitter_profile(access_token))
			      person_hash = {'username' => person['screen_name'],'name' => person['name'], 'provider' => api.category}
			      ...
			      person_hash = {'username'=>facebook_email, 'name'=>person.parsed_response['name'], 'provider' => api.category}
			    elsif api.category == "google"
			      person = Yajl::Parser.parse(google_profile(access_token))
			  ...
			  def google_person_hash(person,api)
			    ...
			    return {'username' => email, 'provider' => api.category, 'name' => name }
			  end
